...
export default function TableWithdraw() {
  ...
  const columns = [
    ...
    { id: 'payee', name: 'المستفيد' },
    { id: 'reasonWithdraw', name: 'السبب' },
  ];
  ...
  const totalAmountWithdraw = filteredRows.reduce((sum, row) => sum + (parseFloat(row.amountWithdraw) || 0), 0);
    ...
    <Grid size={12}>
      <Box sx={{ overflow: 'hidden', backgroundColor: theme.palette.colors.box }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 2 }}>
          ...
          />
          <Typography variant="h6" sx={{fontWeight:'600'}}>الاجمالي : {totalAmountWithdraw.toLocaleString()}</Typography>
        </Box>
        <Paper sx={{ width: "100%"}}>
